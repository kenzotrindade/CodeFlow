  ...
  return (
    <>
      <nav className="fixed top-0 left-0 w-full z-120 px-6 py-4 md:px-12 md:py-8 flex justify-between items-center transition-all duration-300">
        ...
        <Link
          href="/"
          className="text-2xl md:text-4xl font-black tracking-tighter mix-diff"
        >
          CODE
          <span
        ...
        >
          <div className="relative w-8 h-8">
            <Menu
              className={`absolute inset-0 w-8 h-8 transition-all duration-300 ${isOpen ? "opacity-0 rotate-90 scale-50" : "opacity-100 rotate-0 scale-100"}`}
              strokeWidth={1.5}
            />
            <X
              className={`absolute inset-0 w-8 h-8 transition-all duration-300 ${isOpen ? "opacity-100 rotate-0 scale-100" : "opacity-0 -rotate-90 scale-50"}`}
              strokeWidth={1.5}
            />
          </div>
        </button>
      ...
      <div
        className={`fixed inset-0 z-110 min-[1201px]:hidden transition-all duration-500 ease-in-out ${isOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
      >
        ...
        <div className="relative h-full flex flex-col items-center justify-center p-8 space-y-12">
          ...
          <div className="absolute bottom-12 left-1/2 -translate-x-1/2 opacity-20">
            <span className="text-[10px] font-mono tracking-[0.5em] uppercase text-white/40">
              CodeFlow System v4.0
            </span>
          </div>
        </div>
      </div>
